...
missing_df = new_class_obj.stats_count_missing(var_list=['developer'])
missing_df_peek1 = new_class_obj.peek_at_missing(var='developer',
                                                 select_one_panel='202102',
                                                 full_df=False,
                                                 additional_vars=['minInstalls',
                                                                  'score'])

#%%

# is missing due to scraping?
# most apps missing developers cannot be scraped for any useful information
app_details = scraping_apps_according_to_id(missing_df_peek1.index.tolist())

#%%

print(new_class_obj.df.shape)
NDF = new_class_obj.drop_rows(list_of_row_labels=appids_dev)
print(NDF.shape)
...
dup_index_list = new_class_obj.check_duplicate_indices(df_type='appid')
print(len(dup_index_list))
# new_class_obj.print_col_names()

#%%

# DF = new_class_obj.stats_count_missing(
#     var_list=['appId', 'developer', 'released', 'updated', 'size'],
#     consecutive=False,
#     select_one_panel=None,
#     group_by=None)
# DF2 = new_class_obj.peek_at_missing(var='developer',
#                                     select_one_panel='202004')

# comp_dfs, diff_dfs = new_class_obj.check_whether_var_is_time_invariant(
#     var='developer',
#     consecutive=False,
#     format_text=True)
#
# Dict_List, Dict_DF3 = new_class_obj.appids_that_have_var_that_changes_over_time(
#     var='developer',
#     consecutive=False,
#     format_text=True)

diff_dfs, unique_index = new_class_obj.appids_have_time_variant_var(
    var = 'developer',
    consecutive=False,
    format_text=True)
...
print(new_class_obj.df.shape)
NDF = new_class_obj.drop_rows(list_of_row_labels=unique_index)
print(NDF.shape)
...
# why after running the first time, the combined_appid is still not zero?
col_list, panel_list = new_class_obj.select_the_var(var='developer')
ADF = new_class_obj.keep_both_cols_and_rows(list_of_row_labels=combined_appid,
                                            list_of_col_names=col_list)

#%%

# look at those companies that changed developers
for c in Dict_DF3['202004'].columns:
    if 'developer' in c:
        print(c)
DF4 = Dict_DF3['202004'][['developer_202004','developerId_202004',
                          'developer_202009', 'developerId_202009']]

#%%

diff_panels = new_class_obj.check_whether_var_is_time_invariant(
    var='developer',
    consecutive=False)
...
# convert index
DF = new_class_obj.convert_from_appid_to_developerid_index(
...
DFS = new_class_obj.stats_table_dummy(
    var_list=[
    'free_True',
    'adSupported_True',
    'containsAds_True',
    ...
    'contentRating_Teen',
    'genreId_Entertainment',
    'genreId_Game',
    'genreId_Productivity'],
    consecutive=False, select_one_panel=None)

#%%

DFS = new_class_obj.stats_table_numeric(
    var_list=[
        'minInstalls',
        'reviews',
        'score',
...
DFS = new_class_obj.stats_table_datetime(
    var_list=[
        'released',
        'updated'],
    consecutive=False, select_one_panel=None)
...
DFS = new_class_obj.stats_table_address(
    var_list=[developerAddress],
    consecutive=False, select_one_panel=None)

#%%

new_class_df = single_variable_stats(
                              df=DF8,
                              v1='minInstalls',
                              var_type='continuous',
                              initial_panel='201907',
...
# 4.7.2 Single Var Summary Stats (score)
%run -i '2_summary_stats_visualization_new.py'
new_class_df = single_variable_stats(merged_df=DF8,
                              v1='score',
                              var_type='continuous',
                              ...
                              all_panels=all_panels,
                              consec_panels=consec_panels)
histogram = new_class_df.histogram(num_bins=[0,1,2,3,4,5])
s = save_files(fig=histogram)
s.save_fig(fig_name='score_histogram', sub_folder='descriptive_stats')
...
# 4.8 Double Vars Summary Stats (review)
%run -i '2_summary_stats_visualization_new.py'
