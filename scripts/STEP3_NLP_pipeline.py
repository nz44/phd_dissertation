...
class nlp_pipeline():
    ...
    def select_text_cols_into_list(self, consecutive):
        ...
        for j in cols:
            ...
            new_df_list.append(new_df) # a list of pandas series
        return new_df_list
    ...
    def prepare_text_col(self, consecutive=False, combine_panels=True):  # use take_out_the_text_colume_from_merged_df(open_file_func, initial_panel, text_column_name)
        ...
        if combine_panels is True: # here combine all panels' text together to create time invariant text cluster labels
            ...
            processed_panel_text = []
            for single_text_col in new_df_list:
                ...
                single_text_col[single_text_col.columns[0]] = single_text_col[single_text_col.columns[0]].apply(
                    ...
                    lambda x: re.sub(r'[0-9]', '', x)).apply(
                    lambda x: self.remove_stopwords(x))
                processed_panel_text.append(single_text_col)
            return processed_panel_text
    ...
    def tf_idf_transformation(self, consecutive=False,
        ...
        if combine_panels is True:
            ...
            processed_panel_text = self.prepare_text_col(consecutive=consecutive,
                                                    combine_panels=combine_panels)
            matrix_list = []
            for col in processed_panel_text:
                matrix_list.append(self.tf_idf_transformation_single(col.squeeze()))
            return matrix_list
    ...
    def find_optimal_svd_component_plot(self, consecutive=False,
                                     combine_panels=True):
        if combine_panels is True:
            ...
            matrix_list = self.tf_idf_transformation(
                consecutive=consecutive,
                combine_panels=combine_panels
            )
    ...
    def truncate_svd(self,
        ...
        if combine_panels is True:
            ...
            svd_truncated_matrix_list = []
            for matrix in matrix_list:
                svd_truncated_matrix_list.append(
                    self.truncate_svd_single(matrix, n_comp, random_state)
                )
            return svd_truncated_matrix_list
    ...
    def find_optimal_cluster_plot(self, n_cluster_list, n_comp, random_state, consecutive=False, combine_panels=True):
        if combine_panels is True:
            ...
            svd_matrix_list = self.truncate_svd(
                n_comp=n_comp,
                random_state=random_state,
                consecutive=consecutive,
                combine_panels=combine_panels)
    ...
    def kmeans_cluster(self,
        ...
        if combine_panels is True:
            ...
            y_kmeans = kmeans.fit_predict(
                svd_matrix)  # put matrix_transformed_df here would generate same result as put matrix_transformed
            return y_kmeans, svd_matrix
        else:
            svd_matrix_list = self.truncate_svd(
                n_comp=n_comp,
                random_state=random_state,
            ...
            for svd_matrix in svd_matrix_list:
                kmeans = KMeans(n_clusters=n_clusters, init=init, random_state=random_state)
                y_means_list.append(
                    kmeans.fit_predict(svd_matrix)
                )  # put matrix_transformed_df here would generate same result as put matrix_transformed
            return y_means_list, svd_matrix_list
